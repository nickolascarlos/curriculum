export default function BrazilFlag(props) {
    return (
      <svg
        xmlns="http://www.w3.org/2000/svg"
        xmlnsXlink="http://www.w3.org/1999/xlink"
        viewBox="-2100 -1470 4200 2940"
      >
        <defs>
          <path
            id="i"
            fillRule="evenodd"
            d="M-31.5 0h33a30 30 0 0030-30v-10a30 30 0 00-30-30h-33zm13-13h19a19 19 0 0019-19v-6a19 19 0 00-19-19h-19z"
          ></path>
          <path
            id="n"
            d="M-15.75-22C-15.75-15-9-11.5 1-11.5s14.74-3.25 14.75-7.75c0-14.25-46.75-5.25-46.5-30.25C-30.5-71-6-70 3-70s26 4 25.75 21.25H13.5c0-7.5-7-10.25-15-10.25-7.75 0-13.25 1.25-13.25 8.5-.25 11.75 46.25 4 46.25 28.75C31.5-3.5 13.5 0 0 0c-11.5 0-31.55-4.5-31.5-22z"
          ></path>
          <path
            id="l"
            d="M-26.25 0h52.5v-12h-40.5v-16h33v-12h-33v-11H25v-12h-51.25z"
          ></path>
          <path
            id="k"
            d="M-31.5 0h12v-48l14 48h11l14-48V0h12v-70H14L0-22l-14-48h-17.5z"
          ></path>
          <path
            id="d"
            fillRule="evenodd"
            d="M0 0a31.5 35 0 000-70A31.5 35 0 000 0m0-13a18.5 22 0 000-44 18.5 22 0 000 44"
          ></path>
          <path
            id="f"
            fillRule="evenodd"
            d="M-31.5 0h13v-26h28a22 22 0 000-44h-40zm13-39h27a9 9 0 000-18h-27z"
          ></path>
          <path
            id="j"
            d="M0 0h63v-13H12v-18h40v-12H12v-14h48v-13H0z"
            transform="translate(-31.5)"
          ></path>
          <use id="q" transform="scale(15)" xlinkHref="#a"></use>
          <use id="s" transform="scale(10.5)" xlinkHref="#a"></use>
          <use id="r" transform="scale(21)" xlinkHref="#a"></use>
          <use id="o" transform="scale(31.5)" xlinkHref="#a"></use>
          <use id="p" transform="scale(26.25)" xlinkHref="#a"></use>
          <g id="a" fill="#fff">
            <g id="c">
              <path id="b" d="M0-1v1h.5" transform="rotate(18 0 -1)"></path>
              <use transform="scale(-1 1)" xlinkHref="#b"></use>
            </g>
            <use transform="rotate(72)" xlinkHref="#c"></use>
            <use transform="rotate(-72)" xlinkHref="#c"></use>
            <use transform="rotate(144)" xlinkHref="#c"></use>
            <use transform="rotate(216)" xlinkHref="#c"></use>
          </g>
          <g id="m">
            <clipPath id="e">
              <path d="M-31.5 0v-70h63V0zM0-47v12h31.5v-12z"></path>
            </clipPath>
            <use clipPath="url(#e)" xlinkHref="#d"></use>
            <path d="M5-35h26.5v10H5z"></path>
            <path d="M21.5-35h10V0h-10z"></path>
          </g>
          <g id="h">
            <use xlinkHref="#f"></use>
            <path d="M28 0c0-10 0-32-15-32H-6c22 0 22 22 22 32"></path>
          </g>
        </defs>
        <rect width="100%" height="100%" x="-50%" y="-50%" fill="#009b3a"></rect>
        <path fill="#fedf00" d="M-1743 0L0 1113 1743 0 0-1113z"></path>
        <circle r="735" fill="#002776"></circle>
        <clipPath id="g">
          <circle r="735"></circle>
        </clipPath>
        <path
          fill="#fff"
          d="M-2205 1470a1785 1785 0 013570 0h-105a1680 1680 0 10-3360 0z"
          clipPath="url(#g)"
        ></path>
       
      </svg>
    );
  }